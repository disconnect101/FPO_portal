...
class stats:
    ...
    def __init__(self, farmer):
        self.productionYearWise = Produce.objects.filter(owner=farmer.user).values('date__year').annotate(amount=Sum('amount'), income=Sum('income'))
        self.cropWiseProduction = Produce.objects.filter(owner=farmer.user).values('crop__code').annotate(amount=Sum('amount')).filter(date__year=datetime.now().year)
